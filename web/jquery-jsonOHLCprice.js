...
function drawVisualization() {
	...
	var chart = new google.visualization.ChartWrapper({
		...
		options: {
			...
			chartArea: {
				...
				backgroundColor:{fill: 'white', stroke: 'black', strokeWidth: 1}
			},
			colors: ['black'],
			seriesType: 'candlesticks', 
			bar: {groupWidth: '100%'},
			vAxes: {
				0:{title: '価格', gridlines: {count: 6}, slantedText: 'false'}, 
				1:{title: '出来高', gridlines: {count: 6}, maxValue: 5000}
			},
			hAxis: {
			...
			series: {
				...
				2: {type: 'area', color: 'gold', targetAxisIndex: 1},
				3: {type: 'area', color: 'pink', targetAxisIndex: 1}
			}, 
	...
	// 4本値フォーマット
	new google.visualization.NumberFormat({pattern: '安値 #,###.#'}).format(data, 5);
	new google.visualization.NumberFormat({pattern: '始値 #,###.#'}).format(data, 6);
	new google.visualization.NumberFormat({pattern: '終値 #,###.#'}).format(data, 7);
	new google.visualization.NumberFormat({pattern: '高値 #,###.#'}).format(data, 8);
